...
class MongoOperations {
	...
	public function getSynonymsFromText($text){
        $mongo = new MongoOperations();
        $array_result = array();
        $arrayLimatiser = $mongo->myLemma($text);

        foreach($arrayLimatiser as $metadata){
            $propo = new Proposition($metadata);
            $array_result = array_merge($array_result,$propo->getPsynonymes());
        }
        return $array_result;
    }
}
